...
filename = "data/ric_towing.csv"
ac_url = f"https://apps.richmondgov.com/applications/towviewing/Home/TowViewing?_={int(time.time())}"
...
def jq_datatables(raw_html, table_id): 
    ...
    # special cases
    details = body.find_all(attrs={"class": "tblTowsListing"})
    if len(details) > 0:
...
try:
    tow_df = jq_datatables(r.text, 'tblTowsListing')
except Exception as e:
    logging.error(traceback.format_exc())
    exit(1)

# save raw copy
tow_df.to_csv(f"data/tow_{time.strftime('%Y%m%d-%H%M%S')}.csv",index=False)
